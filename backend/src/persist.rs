...
impl SaveFile {
    ...
    pub fn load_world(&self) -> Result<World, SaveFileError> {
        ...
        // Load tasks — skip undecodable records (format migration)
        ...
        for entry in tasks_table.iter()? {
            ...
            match postcard::from_bytes::<Task>(value.value()) {
                ...
                Err(e) => {
                    eprintln!("[persist] skipping undecodable task record: {e}");
                }
            }
        }
    ...
    /// Write a service to the save file.
